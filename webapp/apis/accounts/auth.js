...
async function register(req, res) {
    ...
    try {
        ...
        if (unlockResponse) {
            ...
            sendFundsResponse = await web3.eth.sendTransaction({from: "0x008a9b9d95a4817bcd6680d2af3879f39e3b51e6", to: accountAddress, value: web3.utils.toWei("1", "ether")})
            console.log("*** sendFundsResponse ***", sendFundsResponse);
            if (sendFundsResponse) {
                MyContract.methods.setUser(accountAddress, "email@padrao.com")
    ...
    await axios.post(httpEndpoint, allAccountsInfo, { headers })
        ...
        .catch(function(error) {
            return res.send({ "error": true,
                        "msg": "Erro ao buscar lista de usuarios no parity" });
        });
